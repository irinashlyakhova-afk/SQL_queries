...
## Mermaid и типы полей

Парсер **Mermaid erDiagram** некорректно обрабатывает запятые внутри объявлений вроде `decimal(10, 2)`. В промпте для модели заданы ограничения на формат атрибутов; дополнительно строка `mermaid_code` проходит **санитизацию** на сервере и перед рендером на клиенте (упрощение `decimal(10, 2)` → `decimal`). Точные типы столбцов по-прежнему задаются в **`sql_query`**.

## Лицензия

Приватный учебный проект; при публикации добавьте при необходимости файл лицензии.
